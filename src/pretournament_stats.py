...
def get_team_stats():
    ...
    for index1 in range(len(all_years)):
        ...
        for index2 in range(len(all_teams)):
            ...
            team = name_to_url(all_teams[index2])
            tournament_url = f"https://www.sports-reference.com/cbb/schools/{team}/men/{year}-gamelogs.html"
            ...
            stat_dict = {"team_game_result": 0, "efg_pct": 0, "fga": 0, "fg_pct": 0, "fg3a": 0, "fg3_pct": 0, "fta": 0, "ft_pct": 0, "orb": 0, "drb": 0, "tov": 0, "opp_name_abbr": [], "pt_diff": 0}
            ...
            for key in stat_dict.keys():
                if key == "team_game_result":
                    ...
                    for game in games:
                        ...
                        team = team_a.find("a").text
                        stat_dict[key].append(team)
                elif key == "pt_diff":
                    score_diff_list = []
                    for game in games:
            ...
            stat_df.loc[index2] = [all_teams[index2], *stat_dict.values()]
...
def get_2025_stats(team_url,team1, team_data):
    stat_df = pd.DataFrame(columns=["Team", "team_game_result", "fg_pct", "fg3_pct", "ft_pct", "efg_pct", "fga",
                                    "fg3a", "fta", "orb", "drb", "tov", "opp_name_abbr", "pt_diff"])

    tournament_url = f"https://www.sports-reference.com/cbb/schools/{team_url}/men/2025-gamelogs.html"
    ...
    games = games[stop_point - 5:stop_point]
    ...
    stat_df.loc[0] = [team1, *stat_dict.values()]

    new_df = pd.merge(stat_df, team_data, on="Team", how="inner")
